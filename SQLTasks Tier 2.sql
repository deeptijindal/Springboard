...
query = '''SELECT DISTINCT f.name FROM Facilities AS f WHERE f.facid NOT IN (SELECT b.facid FROM Bookings AS b WHERE b.memid = 0) ORDER BY f.name'''
df = pd.read_sql_query(query, connection)
print(df)

/* Q13: Find the facilities usage by month, but not guests */
SELECT DISTINCT f.name, MONTH(b.starttime) AS monthUsed FROM `Bookings` AS b 
INNER JOIN `Facilities` AS f ON f.facid = b.facid
